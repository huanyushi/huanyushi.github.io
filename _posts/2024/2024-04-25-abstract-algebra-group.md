...
1. Any group $G$ has two subgroups: $H=G$ and $H = \qty{e}$; the latter is called the *trivial subgroup*.
2. $\ZZ \leq \QQ$ and $\QQ\leq \RR$ with the operation of addition.
3. Let $H = \qty{1,r,r^2,\dots,r^{n-1}}$, then $H\leq D_n=\expval{r,s}$.
4. Let $G$ and $H$ be groups and let$\varphi:G\to H$ be a homomorphism, then $\mathrm{im} \ \varphi\leq H$, $\mathrm{ker}\ \varphi \leq G$.
5. If $K\leq H$ and $H\leq G$, then $K\leq G$.
</details>
...
3. If $\abs{H} = n < \infty$, then for any $a >0$ and $a\vert n$, there is a unique subgroup of $H$ of order $a$. It is $\expval{x^d}$, where $d = \frac{n}{a}$. Furthermore, for $m\in \ZZ$, $\expval{x^m} = \expval{x^{(n,m)}}$.
</div>
