...
HTML 不会再有版本6、7、8……制定标准的人已经把该规范变成一个“活”的标准，会随着技术的发展形成相应文档，所以不再有版本号，甚至我们可以把 HTML5 就称为 HTML。这里的关键是**向后兼容性** (backwards compatibility)，表示我们可以继续向 HTML 增加新的内容，但浏览器还会继续支持原来的新内容。
...
```
>注意：指定 `doctype` 以及用 `<meta>` 标记指定字符编码有点像纳税，这是我们的义务，必须执行。另外对于中文网页也需要声明 `utf-8` 编码，否则会出现乱码。 
...
>注意：域名和网站名并不等同，google&#46;com 是一个域名，而 www.google&#46;com 是一个网站名。购买域名就像买下一块地，比如 google&#46;com。但在这块地上可以建很多网站，例如 home.google&#46;com, outhouse.google&#46;com 等，其中 www.google&#46;com 只是 google&#46;com 域上的一个网站。
...
其中：
- 第一部分 `http` 指出用来获取资源的协议。HTTP 也称为超文本传输协议 (HyperText Transfer Protocol)，这里超出本文的范围，具体在此不做介绍。
